...
Adapter.prototype.query = function( sql, bind, callback )
{
  ...
  var isInsert = false;
  if ( sql.toLowerCase().indexOf('insert') !== -1 && sql.toLowerCase().indexOf('returning id') === -1) {
    isInsert = true;
  }  
  ...
  if (isInsert) {
    queryOperations.push(function( lastResult, callback ) {
      _dbClient.query( 'SELECT lastval();', function( err, results ) {
        if ( err ) {
          if ( !err[ 'code' ] || err.code != '55000' ) {
            // err.code = '55000' is thrown when there is no last val for the session
            // this can occur when the the insert specifies the id
            return callback( err, lastResult );
          }
          err = null;
        } else {
          me._lastInsertId = results.rows[0].lastval;
        }
